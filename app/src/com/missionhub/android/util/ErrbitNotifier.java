/*
    MissionHub Errbit Notifier for Android
    Copyright (c) 2012 Chris Roemmich <chris.roemmich@cojourners.com>
...
public class ErrbitNotifier {
    ...
    private static class ErrbitExceptionHandler implements UncaughtExceptionHandler {
        ...
        public ErrbitExceptionHandler(final UncaughtExceptionHandler defaultExceptionHandlerIn) {
    ...
    // Register to send exceptions to configured airbrake server
    public static void register() {
        if (Configuration.isErrbitEnabled()) {
            register(Application.getContext(), Configuration.getErrbitApiUrl(), Configuration.getErrbitApiKey(), Configuration.getEnvironment().name());
        }
    }

    // // Register to send exceptions to airbrake
    // private static void register(final Context context, final String endpoint, final String apiKey) {
    ...
    private static void register(final Context context, final String endpoint, final String apiKey, final String environmentName, final boolean notifyOnlyProduction) {
        ...
        if (!(currentHandler instanceof ErrbitExceptionHandler) && (environmentName.equals(ENVIRONMENT_PRODUCTION) || !ErrbitNotifier.notifyOnlyProduction)) {
     ...
     */
    public static void setExtraData(final Map<String, String> extraData) {
    ...
    private static void writeExceptionToDisk(final Throwable e, final Map<String, String> metaData) {
        try {
            // Set up the output stream
            final int random = new Random().nextInt(99999);
            final String filename = filePath + versionName + "-" + String.valueOf(random) + ".xml";
            ...
            while (currentEx != null) {
                ...
                // report
                try {
                    final StackTraceElement[] stackTrace = currentEx.getStackTrace();
                ...
                } catch (final Throwable innerException) {
            ...
            if (extraData != null && !extraData.isEmpty()) {
                for (final Map.Entry<String, String> extra : extraData.entrySet()) {
                    s.startTag("", "var");
            ...
            if (metaData != null && !metaData.isEmpty()) {
                for (final Map.Entry<String, String> extra : metaData.entrySet()) {
                    s.startTag("", "var");
                    s.attribute("", "key", extra.getKey());
                    s.text(extra.getValue());
                    s.endTag("", "var");
    ...
    private static void sendExceptionData(final File file) {
        try {
            boolean sent = false;
            final URL url = new URL(errbit_endpoint);
    ...
    private static synchronized void flushExceptions() {
        final File exceptionDir = new File(filePath);
        if (exceptionDir.exists() && exceptionDir.isDirectory()) {
            final File[] exceptions = exceptionDir.listFiles();
            for (final File f : exceptions) {
