...
public class Person {
    ...
    public synchronized Map<Long, Long> getBasicPermissions() {
        if (mPermissions != null) {
            ...
            try {
                ...
                for (OrganizationalPermission permission : permissions) {
                    permission.refresh();
                    temp.put(permission.getOrganization_id(), permission.getId());
                }
            } catch (final DaoException e) {
                getBasicPermissions();
            }
    ...
    public synchronized boolean hasBasicPermission(long permissionId, long organizationId) {
        return getBasicPermissions().get(organizationId) == permissionId;
    }

    public synchronized boolean hasPermission(long permissionId) {
        return hasPermission(permissionId, Session.getInstance().getOrganizationId());
    }

    public synchronized boolean hasPermission(long permissionId, long organizationId) {
        if (daoSession == null) {
            throw new DaoException("Entity is detached from DAO context");
        }
    ...
    public synchronized boolean isAdminOrUser() {
        return isAdminOrUser(Session.getInstance().getOrganizationId());
    }

    public synchronized boolean isAdminOrUser(final long organizationId) {
    ...
    public synchronized boolean isUser(final long organizationId) {
