...
public class FollowupComment {
    ...
    public Person getCommenter() {
        Long __key = this.commenter_id;
        if (commenter__resolvedKey == null || !commenter__resolvedKey.equals(__key)) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            PersonDao targetDao = daoSession.getPersonDao();
            Person commenterNew = targetDao.load(__key);
            synchronized (this) {
                commenter = commenterNew;
            	commenter__resolvedKey = __key;
            }
        }
        return commenter;
    }
    ...
    public List<Rejoicable> getRejoicables() {
        if (rejoicables == null) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            RejoicableDao targetDao = daoSession.getRejoicableDao();
            List<Rejoicable> rejoicablesNew = targetDao._queryFollowupComment_Rejoicables(id);
            synchronized (this) {
                if(rejoicables == null) {
                    rejoicables = rejoicablesNew;
                }
            }
        }
        return rejoicables;
	...
	public void deleteWithRelations() {
		...
		daoSession.getRejoicableDao().deleteByKeyInTx(daoSession.getRejoicableDao().queryBuilder().where(RejoicableDao.Properties.Followup_comment_id.eq(getId())).<Long>listKeys());
		delete();
	}
    // KEEP METHODS END
