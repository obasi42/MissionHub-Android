...
//
// Modified for use in MissionHub 2012 C.Roemmich
...
/**
 * EasyTracker is a class designed to easy the burden of adding tracking code to your application. Simply add this class
 * to your application and extend TrackedActivity instead of Activity for each Activity in your application.
 * <p>
 * This class is designed to be used from a single Thread. Use on the Main UI Thread is acceptable.
 * <p>
 * Note that all of your Activities must extend TrackedActivity (or an equivalent Activity, like TrackedListActivity
 * instead of ListActivity) for this Class to properly track application usage and time. If you have an Activity Class
 * that doesn't extend one provided in this package, you can create one by copying TrackedActivity and having it extend
 * the Activity you want. For example, if you have an Activity subclass called FragmentActivity, simply create a class
 * called TrackedFragmentActivity and have your classes extend that instead of FragmentActivity. The code for
 * TrackedFragmentActivity should be the same as the code in TrackedActivity.
 * <p>
 * You can turn on tracking by providing a String resource of the name ga_api_key with a value of your account id (form
 * UA-12345-6). You can provide various parameters as String, Bool or Integer resources (such as sampleRate) as well.
 * Just use the proper type for the parameter (String for String, Bool for boolean and Integer for int).
 * <p>
 * If you want to track your Activities as well as the application, you can add the Bool resource
 * ga_auto_activity_tracking and give it a value of "true".
 */
public class EasyTracker {
	...
	// dispatching.
	private final int gaDispatchPeriod = 60;
	 ...
	 * 
	 * @return the GoogleAnalyticsTrackerDelegate object, or null if tracking is disabled
	 */
	...
	private void loadParameters() {
		gaAccountId = Configuration.getAnalyticsKey();
		gaEnabled = Configuration.isAnalyticsEnabled();
		gaDebug = Configuration.isAnalyticsDebug();
		gaDryRun = Configuration.isAnalyticsDryRun();
		gaAnonymizeIp = Configuration.isAnalyticsAnonymizeIp();
		gaSampleRate = 100;
	...
	/**
	 * Sets the context to use to the applicationContext of the Context ctx. If the input is not null, this method will
	 * then go on to initialize the EasyTracker Class with parameters from the resource files. If there is an accountId
	 * specified, this method will enable Google Analytics tracking and start up the database Thread. If not, it will
	...
	/**
	 * Track the start of an Activity, but only if autoActivityTracking is true. This method will start a new session if
	 * necessary, and will send an empty event to Google Analytics if autoActivityTracking is false to ensure proper
	 * application-level tracking. Developers should not call this method directly. Extend TrackedActivity (or its other
	 * cousins) instead of Activity to use this method. Note that this method should be called from the Activity's
	 * onStart method.
	 * 
	 * @param activity
	...
	/**
	 * Track Activity restarts due to configuration changes (i.e. orientation change). There is no need to start a new
	 * session in this case. Note that this method should be called from the Activity's onRetainNonConfigurationInstance
	 * callback.
	 * <p>
	 * Note that the GoogleAnalytics SDK supports Android versions back to 1.5. The onRetainNonConfigurationInstance
	 * method is deprecated in Android 3.0, but its replacement is only supported in Android 2.1 and beyond.
	 */
	...
	/**
	 * Track the end of an Activity and/or application. This is done by sending an empty event to Google Analytics. Note
	 * that this method should be called from the Activity's onStop callback.
	 * 
	 * @param activity
	 *            the Activity that is to be tracked
	 */
	...
	/**
	 * Look up the Activity's display name (as defined in a String resource named for the Activity's canonicalName).
	 * 
	 * @param activity
	...
	private String getActivityName(final Activity activity) {
		return activity.getClass().getCanonicalName().replace("com.missionhub.", "");
	}
	...
	/**
	 * Adds an Item to the Transaction identified by Item.orderId. A new Transaction will be created if one doesn't
	 * already exist. This call will overwrite an existing Item object with the same orderId and itemSKU.
	 * 
	 * @param item
	 *            the Item to add
	 */
	...
	/**
	 * Adds a Transaction to be sent to Google Analytics. If a Transaction with the same orderId already exists, it will
	 * be replaced with this Transaction.
	 * 
	 * @param transaction
	...
	/**
	 * Clears all pending Transactions and Items from the internal queue. This method will not affect Transactions and
	 * Items already sent with the trackTransactions() call.
	 */
	...
	/**
	 * Set the campaign referral to the values in the input. If the input is not valid, the method fails. If the input
	 * is valid, the referrer information is changed and a new session is started. Each hit after a successful call to
	 * setReferrer will have the referrer information attached.
	 * 
	 * @param referrer
	...
	/**
	 * Start a new session using the parameters stored in the EasyTracker Class. This method flags
	 * GoogleAnalyticsTracker to start a new session with the next hit it generates. As such, calling this multiple
	 * times in a row will have the same effect of calling it once.
	 */
	...
	/**
	 * Sends all the pending Transactions and Items to dispatch. Once this method is called, all the Transactions and
	 * Items added previously will not be cleared by a clearTransactions call and will eventually be dispatched to the
	 * GoogleAnalytics servers.
	 */
	 ...
	 * 
	 * @param index
	 *            the slot number for the custom var (integer between 1 to 5)
	 * @param name
	 *            the name of the custom var
	 * @param value
	 *            the value for the custom var
	 * @param scope
	 *            the scope of the custom var (1-visitor,2-session,3-page)
	 */
	...
	/**
	 * Queue the GoogleAnalytics call to the database thread, but only if GoogleAnalytics has been enabled.
	 * 
	 * @param r
	...
	/**
	 * All Access to GoogleAnalyticsTracker methods are done on this Thread. This is done as GoogleAnalyticsTracker
	 * makes database calls which should be done off the Main UI Thread. It's also done in order to preserve the order
	 * of those calls.
	 */
	private class TrackerThread extends Thread {
		...
		/**
		 * Simply pull Runnables from the Queue trackerQueue and call their run methods, blocking until there is
	...
	public static class GoogleAnalyticsTrackerDelegateImpl implements GoogleAnalyticsTrackerDelegate {

		private final GoogleAnalyticsTracker tracker = GoogleAnalyticsTracker.getInstance();

		@Override
		public void startNewSession(final String accountId, final int dispatchPeriod, final Context ctx) {
		...
		@Override
		public boolean setCustomVar(final int index, final String name, final String value, final int scope) {
			return tracker.setCustomVar(index, name, value, scope);
		}

		@Override
		public boolean setCustomVar(final int index, final String name, final String value) {
