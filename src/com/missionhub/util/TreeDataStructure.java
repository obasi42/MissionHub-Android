...
public class TreeDataStructure<T> {
    ...
    private ArrayList<TreeDataStructure<T>> leafs = new ArrayList<TreeDataStructure<T>>();
    ...
    public TreeDataStructure<T> addLeaf(final T leaf) {
        TreeDataStructure<T> t = new TreeDataStructure<T>(leaf);
        leafs.add(t);
        t.parent = this;
    ...
    public TreeDataStructure<T> setAsParent(final T parentRoot) {
        TreeDataStructure<T> t = new TreeDataStructure<T>(parentRoot);
        t.leafs.add(this);
        this.parent = t;
        t.locate = this.locate;
    ...
    public Collection<T> getSuccessors(final T root) {
        ...
        TreeDataStructure<T> tree = getTree(root);
        if (null != tree) {
            for (TreeDataStructure<T> leaf : tree.leafs) {
     ...
     * @return the successors
     */
    public static <T> Collection<T> getSuccessors(final T of,
            final Collection<TreeDataStructure<T>> in) {
        for (TreeDataStructure<T> tree : in) {
    ...
    private String printTree(final int increment) {
        ...
        s = inc + head.toString();
        for (TreeDataStructure<T> child : leafs) {
