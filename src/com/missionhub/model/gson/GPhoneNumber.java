		...
		final Callable<List<PhoneNumber>> callable = new Callable<List<PhoneNumber>>() {
			...
			public List<PhoneNumber> call() throws Exception {
				synchronized (allLock) {
					...
					final LazyList<PhoneNumber> delete = dao.queryBuilder().where(PhoneNumberDao.Properties.Person_id.eq(personId)).listLazyUncached();
					final CloseableListIterator<PhoneNumber> itr = delete.listIteratorAutoClose();
					while (itr.hasNext()) {
						dao.delete(itr.next());
					}
