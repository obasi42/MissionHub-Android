...
public class EducationDao extends AbstractDao<Education, Long> {
	...
	public EducationDao(final DaoConfig config) {
		super(config);
	}

	public EducationDao(final DaoConfig config, final DaoSession daoSession) {
		super(config, daoSession);
	}

	/** Creates the underlying database table. */
	public static void createTable(final SQLiteDatabase db, final boolean ifNotExists) {
		final String sql = "CREATE TABLE " + (ifNotExists ? "IF NOT EXISTS " : "") + "'EDUCATION' (" + //
	...
	public static void dropTable(final SQLiteDatabase db, final boolean ifExists) {
		final String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "'EDUCATION'";
		db.execSQL(sql);
	}

	/** @inheritdoc */
	@Override
	protected void bindValues(final SQLiteStatement stmt, final Education entity) {
		stmt.clearBindings();
		...
		final String school_name = entity.getSchool_name();
		if (school_name != null) {
			stmt.bindString(3, school_name);
		}

		final String school_id = entity.getSchool_id();
		if (school_id != null) {
			stmt.bindString(4, school_id);
		}

		final String year_name = entity.getYear_name();
		if (year_name != null) {
			stmt.bindString(5, year_name);
		}

		final String year_id = entity.getYear_id();
		if (year_id != null) {
	...
	/** @inheritdoc */
	@Override
	public Education readEntity(final Cursor cursor, final int offset) {
		final Education entity = new Education( //
	...
	/** @inheritdoc */
	@Override
	public void readEntity(final Cursor cursor, final Education entity, final int offset) {
	...
	@Override
	protected Long updateKeyAfterInsert(final Education entity, final long rowId) {
		entity.setId(rowId);
		return rowId;
	}
	...
	/** @inheritdoc */
	@Override
	public Long getKey(final Education entity) {
		if (entity != null) {
			return entity.getId();
		} else {
			return null;
	...
	/**
	 * Internal query to resolve the "educationList" to-many relationship of
	 * Person.
	 */
	public synchronized List<Education> _queryPerson_EducationList(final Long person_id) {
		if (person_EducationListQuery == null) {
			final QueryBuilder<Education> queryBuilder = queryBuilder();
