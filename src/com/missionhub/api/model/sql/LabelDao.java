...
public class LabelDao extends AbstractDao<Label, Long> {
	...
	private Query<Label> group_LabelsQuery;

	public LabelDao(final DaoConfig config) {
		super(config);
	}

	public LabelDao(final DaoConfig config, final DaoSession daoSession) {
	...
	public static void createTable(final SQLiteDatabase db, final boolean ifNotExists) {
		final String sql = "CREATE TABLE " + (ifNotExists ? "IF NOT EXISTS " : "") + "'LABEL' (" + //
	...
	public static void dropTable(final SQLiteDatabase db, final boolean ifExists) {
		final String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "'LABEL'";
		db.execSQL(sql);
	}

	/** @inheritdoc */
	@Override
	protected void bindValues(final SQLiteStatement stmt, final Label entity) {
		stmt.clearBindings();
	...
	@Override
	protected void attachEntity(final Label entity) {
	...
	/** @inheritdoc */
	@Override
	public Label readEntity(final Cursor cursor, final int offset) {
		final Label entity = new Label( //
	...
	/** @inheritdoc */
	@Override
	public void readEntity(final Cursor cursor, final Label entity, final int offset) {
	...
	@Override
	protected Long updateKeyAfterInsert(final Label entity, final long rowId) {
		entity.setId(rowId);
		return rowId;
	}
	...
	/** @inheritdoc */
	@Override
	public Long getKey(final Label entity) {
		if (entity != null) {
			return entity.getId();
		} else {
			return null;
	...
	/** Internal query to resolve the "labels" to-many relationship of Group. */
	public synchronized List<Label> _queryGroup_Labels(final Long organization_id) {
		if (group_LabelsQuery == null) {
			final QueryBuilder<Label> queryBuilder = queryBuilder();
			queryBuilder.where(Properties.Organization_id.eq(organization_id));
			group_LabelsQuery = queryBuilder.build();
		} else {
			group_LabelsQuery.setParameter(0, organization_id);
		}
		return group_LabelsQuery.list();
	}
	...
	protected Label loadCurrentDeep(final Cursor cursor, final boolean lock) {
		final Label entity = loadCurrent(cursor, 0, lock);
	...
	public Label loadDeep(final Long key) {
		assertSinglePk();
		if (key == null) {
			return null;
		}
	 ...
	 * ImageTO objects.
	 */
	public List<Label> loadAllDeepFromCursor(final Cursor cursor) {
		final int count = cursor.getCount();
		final List<Label> list = new ArrayList<Label>(count);
	...
	protected List<Label> loadDeepAllAndCloseCursor(final Cursor cursor) {
		try {
	...
	public List<Label> queryDeep(final String where, final String... selectionArg) {
