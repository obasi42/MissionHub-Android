...
public class OrganizationRoleJsonSql {
	...
	public static void update(final Context context, final int personId, final GOrgGeneric[] roles, final String tag) {
		if (roles == null)
			return;
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				for (GOrgGeneric role : roles) {
					...
					if (tag != null)
						b.putString("tag", tag);
					app.getApiNotifier().postMessage(ApiNotifier.Type.UPDATE_ORGANIZATION_STUB, b2);
				}
			}
		});
