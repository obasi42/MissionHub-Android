		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				for (GKeyword keyword : keywords) {
					...
					List<KeywordQuestion> keywordQuestions = kqd.queryBuilder().where(Properties.Keyword_id.eq(keyword.getId())).list();
					...
					if (keyword.getQuestions() != null) {
						QuestionJsonSql.update(context, k.getOrganization_id(), keyword.getQuestions(), tag);
						for (GQuestion question : keyword.getQuestions()) {
							KeywordQuestion kq = new KeywordQuestion();
							kq.setKeyword_id(keyword.getId());
							kq.setQuestion_id(question.getId());
