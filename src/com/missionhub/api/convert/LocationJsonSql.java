...
public class LocationJsonSql {
	...
	public static void update(final Context context, final long personId, final GIdNameProvider location, final boolean threaded, final boolean notify,
	...
	private static void privateUpdate(final Context context, final long personId, final GIdNameProvider location, final boolean threaded, final boolean notify,
		...
		final LocationDao ld = app.getDbSession().getLocationDao();

		// delete current interests in db
		final LazyList<Location> currentInterests = ld.queryBuilder().where(Properties.Person_id.eq(personId)).listLazy();
